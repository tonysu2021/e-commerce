...
public class BizConsumer {
	...
	private BizReplyProducer bizReplyProducer;

	@EventHandler(target = StreamChannel.ORDER_INPUT, eventType = "TEST")
	public void receiveMsgFromOrder(@Header(name = StreamHeader.X_SERVER_ID, required = true) UUID serverId,
		...
		helper.doInTransaction(StreamChannel.ORDER_INPUT, messageId, type, attempt, payload, item -> {
			...
			if (StringUtils.contains(msg, "pass-b")) {
				throw new ImmediateAcknowledgeAmqpException(ExceptionCodeEnum.DOMAIN_ERROR.getMsg());
			}
		});
	}
	
	@EventHandler(target = StreamChannel.ORDER_PRIORITY_INPUT, eventType = "TEST")
	public void receiveMsgFromOrderP(@Header(name = StreamHeader.X_SERVER_ID, required = true) UUID serverId,
		...
		helper.doInTransaction(StreamChannel.ORDER_PRIORITY_INPUT, messageId, type, attempt, payload, item -> {
			...
			bizReplyProducer.sendToOrderReply(serverId, messageId, StreamActionType.TEST_REPLY, item);
		});
	}
}